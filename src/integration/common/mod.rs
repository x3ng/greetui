...
impl IntegrationRunner {
  pub async fn new(opts: SessionOptions, builder: Option<fn(&mut App)>) -> IntegrationRunner {
    IntegrationRunner::new_with_size(opts, builder, (200, 40)).await
  }

  pub async fn new_with_size(opts: SessionOptions, builder: Option<fn(&mut App)>, size: (u16, u16)) -> IntegrationRunner {
    ...
    let client = tokio::task::spawn(async move {
      let mut config = Config::try_parse_from::<[&str; 0], &str>([]).unwrap_or_default();
      // Override socket path for testing
      std::env::set_var("GREETD_SOCK", socket.to_str().unwrap());

      let mut app = App::new(config, events.sender()).await;
      app.sessions.source = SessionSource::Command("uname".to_string());

      if let Some(builder) = builder {
        builder(&mut app);
      }

      app.socket = socket.to_str().unwrap().to_string();
      app.events = Some(events.sender());
      app.connect().await;

      let _ = crate::run(backend, app, events).await;
    });
  ...
  pub async fn join_until_client_exit(&mut self, mut events: JoinHandle<()>) {
    ...
    assert!(exited, "greetui did not exit");
  }
