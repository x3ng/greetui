...
async fn show_greet() {
  ...
  let mut runner = IntegrationRunner::new(
    opts,
    Some(|app| {
      app.ui.greeting = Some("Lorem ipsum dolor sit amet".to_string());
    }),
  )
  .await;
...
async fn show_wrapped_greet() {
  ...
  let mut runner = IntegrationRunner::new_with_size(
    opts,
    Some(|app| {
      app.ui.greeting = Some("Lorem \x1b[31mipsum dolor sit amet".to_string());
    }),
    (20, 20),
  )
  .await;
  ...
  let events = tokio::task::spawn({
    ...
    async move {
      ...
      assert!(output.contains("sit amet"));
...
async fn show_time() {
  ...
  let mut runner = IntegrationRunner::new(
    ...
    Some(|app| {
      app.ui.time = true;
      app.ui.time_format = Some(TIME_FORMAT.to_string());
    }),
  )
  .await;
