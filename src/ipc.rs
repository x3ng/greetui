...
impl Ipc {
  ...
  pub async fn handle(&mut self, app: Arc<RwLock<App>>) -> Result<(), Box<dyn Error>> {
    ...
    if let Some(request) = request {
      let stream = {
        ...
        app.stream.as_ref().unwrap().clone()
      };
      ...
      let response = {
        ...
        app.write().await.working = false;

        response
      };

      self.parse_response(&mut *app.write().await, response).await?;
    }

    Ok(())
  }

  async fn parse_response(&mut self, app: &mut App, response: Response) -> Result<(), Box<dyn Error>> {
    ...
    match response {
      Response::AuthMessage { auth_message_type, auth_message } => match auth_message_type {
        AuthMessageType::Secret => {
          ...
          app.working = false;
          app.auth.asking_for_secret = true;
          app.set_prompt(&auth_message);
        }

        AuthMessageType::Visible => {
          app.auth.mode = Mode::Password;
          app.working = false;
          app.auth.asking_for_secret = false;
          app.set_prompt(&auth_message);
        }

        AuthMessageType::Error => {
          app.ui.message = Some(auth_message);
        ...
        AuthMessageType::Info => {
          ...
          app.auth.previous_mode = app.auth.mode;
          app.auth.mode = Mode::Action;

          if let Some(message) = &mut app.ui.message {
            message.push('\n');
            message.push_str(auth_message.trim_end());
          } else {
            app.ui.message = Some(auth_message.trim_end().to_string());
          }
      ...
      Response::Success => {
        if app.done {
          ...
          match app.sessions.source.command(app).map(str::to_string) {
            None => {
              Ipc::cancel(app).await;

              app.ui.message = Some(fl!("command_missing"));
            ...
            Some(command) => {
              ...
              let session = Session::get_selected(app);
              let default = DefaultCommand(&command, app.sessions.source.env());
              let (command, env) = wrap_session_command(app, session, &default);
      ...
      Response::Error { error_type, .. } => {
        ...
        match error_type {
          ErrorType::AuthError => {
            app.ui.message = Some(fl!("failed"));
            self
              .send(Request::CreateSession {
                username: app.auth.username.value.clone(),
              })
              .await;
            app.reset(true).await;
          }

          ErrorType::Error => {
            // Do not display actual message from greetd, which may contain entered information, sometimes passwords.
            app.ui.message = Some("An error was received from greetd".to_string());
            app.reset(false).await;
          }
        }
      }
  ...
  pub async fn cancel(app: &mut App) {
    tracing::info!("cancelling session");

    let _ = Request::CancelSession.write_to(&mut *app.stream().await).await;
  }
}
...
fn wrap_session_command<'a>(app: &App, session: Option<&Session>, default: &'a DefaultCommand<'a>) -> (Cow<'a, str>, Vec<String>) {
  ...
  match session {
    ...
    Some(Session {
      ...
      if *session_type == SessionType::X11 {
        if let Some(ref wrap) = app.sessions.xsession_wrapper {
          return (Cow::Owned(format!("{} {}", wrap, default.command())), env);
        }
      } else if let Some(ref wrap) = app.sessions.session_wrapper {
        return (Cow::Owned(format!("{} {}", wrap, default.command())), env);
      }
    }
    ...
    _ => {
      ...
      if let Some(ref wrap) = app.sessions.session_wrapper {
        return (Cow::Owned(format!("{} {}", wrap, default.command())), env);
      }
...
mod test {
  ...
  fn wayland_no_wrapper() {
    ...
    let (command, env) = wrap_session_command(&app, Some(&session), &default);
  ...
  fn wayland_wrapper() {
    ...
    app.sessions.session_wrapper = Some("/wrapper.sh".into());
  ...
  fn x11_wrapper() {
    let mut app = App::default();
    app.sessions.xsession_wrapper = Some("startx /usr/bin/env".into());
