...
const LAST_USER_USERNAME: &str = "/var/lib/greetui/lastuser";
const LAST_USER_NAME: &str = "/var/lib/greetui/lastuser-name";
const LAST_COMMAND: &str = "/var/lib/greetui/lastsession";
const LAST_SESSION: &str = "/var/lib/greetui/lastsession-path";
...
pub fn get_issue() -> Option<String> {
  ...
  if let Ok(issue) = fs::read_to_string("/etc/issue") {
    // Process each line preserving trailing whitespace (fix for #208)
    let issue = issue
      .replace("\\S", "Linux")
    ...
    // Do NOT trim trailing whitespace — preserve ASCII art alignment (#208)
...
pub fn write_last_username(username: &MaskedString) {
  let _ = fs::create_dir_all("/var/lib/greetui");
...
pub fn get_sessions(app: &App) -> Result<Vec<Session>, Box<dyn Error>> {
  let paths = if app.sessions.paths.is_empty() {
    ...
    if let Ok(entries) = fs::read_dir(path) {
      for entry in entries.flatten() {
        if let Ok(Some(session)) = load_desktop_file(entry.path(), *session_type) {
          ...
          let key = (
            session.slug.clone().unwrap_or_default(),
            session.command.clone(),
          );
          if seen.insert(key) {
            files.push(session);
          } else {
            tracing::info!("skipping duplicate session '{}' from '{}'", session.name, entry.path().display());
          }
        }
      }
    }
  ...
  tracing::info!("found {} sessions (after dedup)", files.len());
